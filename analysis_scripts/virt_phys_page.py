...
#	virtual:physical page-frame number format
#
###############################################################################################
...
sys.path.append('/home/wali/work/cat_exp/set_exp/git/cat_exp/second_exp/bench/script/scripts')
...
class simple_page_file(page_file):
        ...
        def parse(self):
                ...
                try:
                        pfn_array = [int(pfn.split(':')[-1]) for pfn in lines]
                except:
...
def main():
	...
	myFile = simple_page_file('/tmp/bandwidth_print.log')
	myFile.parse()
